...
import { ReviewsGallery } from '@/components/marketing'
...
export default async function ReviewsPage({ params }: PageProps) {
  ...
  return (
    <main className="container-page space-y-16 pb-12 sm:space-y-20 sm:pb-16">
      {/* ── 1. Aggregate hero (FR-RV-1) — full-bleed navy band ──────────── */}
      <section
      ...
      >
        ...
        <div className="band-inner grid grid-cols-1 lg:grid-cols-[1.1fr_1fr]">
          {/* Left: eyebrow + headline + subcopy */}
          <div className="relative py-12 text-hero-ink sm:py-14 lg:pr-10">
            {HERO.eyebrow && (
              <p className="mb-4 font-mono text-xs font-semibold uppercase tracking-[0.16em] text-ember-strong sm:text-sm">
                {HERO.eyebrow}
              </p>
            )}
          ...
          <div className="relative mb-12 flex flex-col justify-center gap-5 overflow-hidden rounded-card border border-hero-line bg-gradient-to-br from-hero-strong to-hero px-8 py-10 sm:px-10 lg:mb-0 lg:rounded-none lg:border-y-0 lg:border-l lg:border-r-0 lg:bg-transparent lg:px-10 lg:py-12">
            ...
            <a
              ...
              className="relative inline-flex w-fit items-center gap-1.5 font-sans text-sm font-semibold text-ember-strong underline-offset-4 hover:underline focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-brand-500 focus-visible:ring-offset-2"
            >

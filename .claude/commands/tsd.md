...
3. **Load project context.** Read these scaffolded docs in order so you do
   _not_ re-ask the user about anything they already describe:
   - `AGENTS.md` — agent platform, skills inventory.
   - `docs/PACKAGES.md` — exact stack: http / data-fetching / state / forms /
     validation / testing / auth choices.
   - `docs/ARCHITECTURE.md` — folder layout, provider tree, conventions.
   - `docs/WORKFLOWS.md` — branch / commit / PR / test workflows.
   - `docs/AUTH_PATTERN.md` — auth flow, redirects, role guards.
...
5. **Ask clarifying questions, one at a time.** Cover only what the BRD and
   ...
   **Do not ask** about stack choices, folder layout, branching, commit
   conventions, auth pattern, or anything else covered by the context docs —
   reference them directly in the TSD instead.
...
6. **Derive a slug** from the BRD filename (strip extension, kebab-case). If
   the slug is ambiguous (e.g. multiple features in one BRD), ask the user
   which slug to use.
...
- Frontend / fullstack auto-detection.

...
// regressions the design (docs/specs/2026-06-15-ai-seo-geo-design.md) calls out.
...
// Balanced-brace extraction of an object literal body following `marker`.
...
check('SEO discoverability surfaces exist', () => {
  ...
  assert(
    missing.length === 0,
    `missing: ${missing.join(', ')}`,
    'robots/sitemap/llms.txt/manifest are how search engines and AI crawlers discover and index the site',
    'recreate the missing surface(s); each reads the seoRoutes registry — see the seo skill'
  )
...
// ─── Predicate 15: no private route key appears in seoRoutes ───────────────────

const PRIVATE_ROUTE_KEYS = ['login', 'signup', 'forgotPassword', 'dashboard', 'profile', 'settings']

check('seoRoutes excludes private route keys', () => {
  const src = read('src/config/routes.ts')
  const body = extractObjectBody(src, 'seoRoutes')
  assert(
    body !== null,
    'could not locate the seoRoutes object in src/config/routes.ts',
    'the harness parses seoRoutes to ensure private routes are never published',
    'keep `export const seoRoutes = { … }` as an object literal in src/config/routes.ts'
  )
  const leaked = PRIVATE_ROUTE_KEYS.filter((k) => new RegExp(`(^|[\\s,{])${k}\\s*:`).test(body))
  assert(
    leaked.length === 0,
    `private route key(s) in seoRoutes: ${leaked.join(', ')}`,
    'a private/auth route in seoRoutes would be published to the sitemap and llms.txt and surfaced to crawlers',
    'remove the listed key(s) from seoRoutes; private routes are excluded by omission and marked noindex'
  )
  return `${PRIVATE_ROUTE_KEYS.length} private keys absent from seoRoutes`
})

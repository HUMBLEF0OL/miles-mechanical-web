...
const QUARTERLY_UPDATE_TAG_PREFIX = 'starter-update-'
const RED = '\x1b[31m'
...
const tags = run('git tag --list "starter-update-*" --sort=-version:refname').trim().split('\n')
const lastTag = tags[0]

if (!lastTag || lastTag === '') {
  console.log(
    `${YELLOW}⚠ No update tags found. This boilerplate may never have been updated.${RESET}`
  )
...
To run the full quarterly update:
  ...
  ${BOLD}# or: open .github/agents/updater.agent.md in your AI tool of choice${RESET}
`)
